...
def new_fun():
    global acquireButton,x_gc,gc_values,x_gcB,gc_valuesB,start_time
...
def myReadValuesB():
    global value_BB,start_timeB,gc_valuesB
    #gc_valuesB = []
    if value_BB == 1:
        start_timeB = time.time()
        value_BB = 2
...
def ReadValuesChannelB(main_stopB):
...
def AcquireIconCallback():
    #print(detectorAObject.myOnOff())
    global A_Flag,B_Flag,startAcquire,yA_values_forSavefile,yB_values_forSavefile,acquire_value,xA_values_forSavefile,xB_values_forSavefile,acquireIcon_state,acquireButton,main_stoping_thread
    yA_values_forSavefile = []
    yB_values_forSavefile = []
    ...
    if detectorBObject.OnOffFlag.get():
        try:
            global values_thread_B
            main_stoping_thread = False
            values_thread_B = threading.Thread(target = ReadValuesChannelB,args = (lambda: main_stoping_thread,))
