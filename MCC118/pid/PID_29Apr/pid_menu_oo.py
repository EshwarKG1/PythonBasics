...
def new_fun():
    global acquireButton,stopButton #,x_gc,gc_values,x_gcB,gc_valuesB,start_time
...
start_timeBB = time.time()
def myReadValuesB():
    global value_BB,start_timeB,gc_valuesB,start_timeBB,board1
    #gc_valuesB = []
    #board1 = board.a_in_scan_start(1)
    if value_BB == 1:
        start_timeB = time.time()
        start_timeBB = start_timeB
        value_BB = 2
    g_varB = board.a_in_read(1)
    g_varB1 = round(g_varB,4)
    gc_valuesB.append(g_varB1)
    PIDGraphObject.bY_plot_values.append(gc_valuesB)
