...
def new_fun():
    global acquireButton,stopButton,read_values #,x_gc,gc_values,x_gcB,gc_valuesB,start_time
...
def FileOpenWrapper():
   # def FileOpenWrapper():
    ...
    if data['Detector A'] == "ON" :
        detectorAObject.OnOffFlag = True
    else:
        detectorAObject.OnOffFlag = False
        
    if data['Detector B'] == "ON" :
        detectorBObject.OnOffFlag = True
    else:
        detectorBObject.OnOffFlag = False
    if detectorAObject.OnOffFlag:
...
def myReadValues():
    ...
    #print("gc_values : ",g_var1)
    #print("length gc_values:",len(gc_values))
    actual_time = time.time()-start_time
    actual_time = round(actual_time,6)
    #print(actual_time)
    x_gc.append(actual_time)
    #print("x_gc :",actual_time)
    #print("Length x_gc :",len(x_gc))
...
def myReadValuesB():
    ...
    if PIDGraphObject.plotValuesFlag == 0:
        PIDGraphObject.plotValuesFlag = -1
        PIDGraphObject.bX_plot_values.append(actual_timeB)
        PIDGraphObject.bY_plot_values.append(g_varB1)
        PIDGraphObject.plotValuesFlag = 0
    #if len(x_gcB) == len(gc_valuesB):
        
    if len(PIDGraphObject.bY_plot_values) > len(PIDGraphObject.bX_plot_values):
        print("Length X values :",len(PIDGraphObject.bX_plot_values))
        print("length Y values:",len(PIDGraphObject.bY_plot_values))
        print("Actual time B :",actual_timeB)
        #gc_valuesB.pop(-1)
    elif len(PIDGraphObject.bY_plot_values) < len(PIDGraphObject.bX_plot_values):
        print("Y values less than X")
...
def AcquireIconCallback():
    #print(detectorAObject.myOnOff())
    global values_thread_A,values_thread_B
...
def Stopcallback():
    global values_thread_A,values_thread_B,read_values
...
def Loadcallback_plot():
    ...
    if data['Detector B'] == 'ON':
        ...
        detectorBObject.OnOffFlag = False
    if detectorAObject.OnOffFlag:
        PIDGraphObject.aX_values = []
        PIDGraphObject.aY_values = []
        with open(data['Detector-A-Data'],'r') as file:
            csvreader = csv.reader(file)
            header = next(csvreader)
            for row in csvreader:
                ...
                #PIDGraphObject.aX_values.append(row[0])
                #PIDGraphObject.aY_values.append(row[1])
                PIDGraphObject.aX_values.append(row_A0)
                PIDGraphObject.aY_values.append(row_A1)
                #print(PIDGraphObject.aX_values)
        PIDGraphObject.PopUpGraphA_Cons()
    if detectorBObject.OnOffFlag:
        PIDGraphObject.bX_values = []
        PIDGraphObject.bY_values = []
        with open(data['Detector-B-Data'],'r') as file:
            csvreader = csv.reader(file)
            header = next(csvreader)
            for row in csvreader:
                ...
                #PIDGraphObject.bX_values.append(row[0])
                #PIDGraphObject.bY_values.append(row[1])
                PIDGraphObject.bX_values.append(row_B0)
                PIDGraphObject.bY_values.append(row_B1)
        PIDGraphObject.PopUpGraphB_Cons()
    return
