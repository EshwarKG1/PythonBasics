...
class startup:
    def __init__(self, master):
        ...
        #     time.sleep(0.2)
        #
        # GPIO.add_event_detect(pins[0], GPIO.FALLING, callback=lambda i: self.start(),bouncetime=bt)
        # GPIO.add_event_detect(pins[1], GPIO.FALLING, callback=lambda i: self.calibrate(),bouncetime=bt)
        # GPIO.add_event_detect(pins[2], GPIO.FALLING, callback=lambda i: self.temppage(),bouncetime=bt)
        # GPIO.add_event_detect(pins[3], GPIO.FALLING, callback=lambda i: self.methodpage(),bouncetime=bt)
    ...
    def GetSensor(self):
        ...
        try:
            ...
            data = board.a_in_read(1)
            #data = adc.read_adc(0, gain=GAIN)
            # data = round(uniform(20.0, 25.0), 2)
            #data_raw = bme280.sample(bus, address, calibration_params)
            #data = round((data_raw.pressure-1005)*3,1)
...
#                            arrowprops=dict(facecolor='black', shrink=0.05), )
    ...
    def save_plot(self,sensor,xs):
        fig = Figure(figsize=(6, 10), dpi=100, facecolor='white')
        raw = sensor
        disp_data = [i - self.noise for i in raw]
        ...
        formatter = matplotlib.ticker.FuncFormatter(lambda s, x: time.strftime('%M:%S', time.gmtime(s)))
        ax.xaxis.set_major_formatter(formatter)
        ax.set_title('GC Report 03/23',fontsize=20)
        ax.set_xlabel('Time mm/ss')
        ax.set_ylabel('Reading()')
        #        ax.set_ylim(-1,30)
        ax.grid(True)
