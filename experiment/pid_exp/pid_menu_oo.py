...
def ReadValuesChannelA():
    ...
    while startAcquire:
        g_var = board.a_in_read(0)
        g_var1 = round(g_var,4)
        #RA_x,RA_Y = myReadValues()
        #PIDGraphObject.aY_plot_values = RA_Y
        PIDGraphObject.aY_plot_values.append(g_var1)
        actual_time = time.time()- start_time
        actual_time = round(actual_time,4)
        #PIDGraphObject.aX_plot_values = RA_x
...
def myReadValuesB():
    ...
    read_values_xB = x_gcB
    read_values_yB = gc_valuesB
    return read_values_xB,read_values_yB

def ReadValuesChannelB():
    global gc_valuesB,startAcquire,start_timeB,x_gcB,value_BB,read_values_xB,read_values_yB
    start_timeB = time.time()
    while startAcquire:
        g_varB = board.a_in_read(1)
        g_varB1 = round(g_varB,4)
        #print(PIDGraphObject.bY_plot_values.append(g_varB1))
        PIDGraphObject.bY_plot_values.append(g_varB1)
        actual_timeB = time.time()- start_timeB
        actual_timeB = round(actual_timeB,4)
        PIDGraphObject.bX_plot_values.append(actual_timeB)
        #print(PIDGraphObject.bX_plot_values.append(actual_timeB))
