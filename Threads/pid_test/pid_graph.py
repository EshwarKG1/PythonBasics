...
def Loadcallback(my_direc):
    ...
    with open(dire,"r") as f:
        ...
        for k in data:
            ...
            #l.append(l_s)
            #if l[0] == 'Time A' and l[1] == 'Detector A' and l[2] == 'Time B' and l[3] == 'Detector B':
                #only_graphAB = 1
                #print("1")
            #print(l)
                #only_graphAB = 2
                #print("2")
            #elif l[0] == 'Time B' and l[1] == 'Detector B':
                #only_graphAB = 3
                #print("3")
    ...
    with open(dire,"r") as f:
        ...
        for k in data:
            l = k.split(",")
                ...
                #only_graphAB = 1
                #print("1")
            if l[0] == 'Time A' and l[1] == 'Detector A':
                only_graphAB = 2
                #print("2")
            elif l[0] == 'Time B' and l[1] == 'Detector B':
                only_graphAB = 3
            ...
            for k_AB in dataAB:
                try:
                    ...
                    if l_AB[0] == '\n':
                        ...
                        aX_values.append(l_AB[0])
                        aY_values.append(l_AB[1])
                        bX_values.append(l_AB[2])
                        bY_values.append(l_AB[3])
                except IndexError:
                    pass
        return aX_values,aY_values,bX_values,bY_values
    """
    if only_graphA == 1 and only_graphB == 1:
        direcA_CSV = os.path.splitext(dire)[0]
        direcA_CSV = direcA_CSV + '-Detector-A.csv'
        with open(direcA_CSV,"r") as fileA:
            ...
            for k_A in dataA:
                ...
                if valuesA_i == 1:
                    ...
                    aX_values.append(l_A[0])
                    aY_values.append(l_A[1])
                    
        direcB_CSV = os.path.splitext(dire)[0]
        direcB_CSV = direcB_CSV + '-Detector-B.csv'
        
        with open(direcB_CSV,"r") as fileB:
            ...
            for k_B in dataB:
                ...
                if valuesB_i == 1:
                    ...
                    #print('0',l_B[0])
                    #print(l_B[1])
                    bX_values.append(l_B[0])
                    bY_values.append(l_B[1])
        return aX_values,aY_values,bX_values,bY_values
    
    elif only_graphA == 1 and only_graphB == 0:
        direcA_CSV = os.path.splitext(dire)[0]
        direcA_CSV = direcA_CSV + '-Detector-A.csv'
        with open(direcA_CSV,"r") as fileA:
            dataA = fileA.readlines()
    ...
    elif only_graphA == 0 and only_graphB == 1:
        direcB_CSV = os.path.splitext(dire)[0]
        direcB_CSV = direcB_CSV + '-Detector-B.csv'
        with open(direcB_CSV,"r") as fileB:
            dataB = fileB.readlines()
...
class PlotGraphClass:
  ...
  def plotA_Cons(self,cons_filename):
      ...
      yB_cons_values = []
      #xA_cons = range(50)
      xA_cons_values,yA_cons_values,xB_cons_values,yB_cons_values = Loadcallback(cons_filename)
      ...
      if yA_cons_values != []:
          ...
          for i in range(len(xA_cons_values)):
              xA_cons_valuesF.append(float(xA_cons_values[i]))
          ...
          subplot.plot(xA_cons_valuesF,yA_cons_valuesF,color='#FF6700')
          #canvas.draw()
      else:
  ...
  def plotB_Cons(self,cons_filename):
      ...
      if yB_cons_values != [] :
          ...
          for i in range(len(xB_cons_values)):
              xB_cons_valuesF.append(float(xB_cons_values[i]))
              
          for i in range(len(yB_cons_values)):
              yB_cons_valuesF.append(float(yB_cons_values[i]))
          #print(yA_cons_values)
          subplot.plot(xB_cons_valuesF,yB_cons_valuesF,color='#00B5FF')
          #canvas.draw()
      else:
