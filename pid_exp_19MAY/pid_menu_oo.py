...
def new_fun():
    ...
    detectorAObject.LengthStr.set("")
    detectorAObject.TimeStr.set("")
    detectorBObject.LengthStr.set("")
    detectorBObject.TimeStr.set("")
    read_values = True
    return
...
def FindCompoundName(retention_time):
    ...
    for row in rows:
        row_int = float(row[2])
        if row_int-4.0 < retention_time < row_int+4.0:
            return row[0]
    return "Unknown"
        ...
        if curveValue_aY != 0:
            ...
            for i,j in zip(data_List_aX,data_List_aY):
                if j>max_data:
                    ...
                    max_data = j
                    peak_rt = round(i,2)  
            peak_heightA = max_data 
            peak_heightA = round(peak_heightA,2)
            peak_areaA = simps(data_List_aY,dx=0.2)
            peak_areaA = round(peak_areaA,2)
...
def myReadValuesB():
    ...
    if g_varB1 < float(detectorBObject.LengthStr.get()):
        g_varB1 = 0
...
def AcquireIconCallback():
    ...
    global A_Flag,B_Flag,startAcquire,yA_values_forSavefile,yB_values_forSavefile
    global acquire_value,xA_values_forSavefile,xB_values_forSavefile,acquireIcon_state,acquireButton
    xA_values_forSavefile = []
    yA_values_forSavefile = []
    xB_values_forSavefile = []
    yB_values_forSavefile = []
...
def CreateIcons():
    ...
    Button(IconLevelMenuBar,image=newIcon,command = new_fun).pack(side=LEFT)
